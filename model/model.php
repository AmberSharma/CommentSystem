...
class MyModel extends model {
   ...
   public function FindUsers($val = "") {
    ...
    public function fetchRoles(){
    	...
    	$this->db->Fields(array("screen_id","permit"));
    	$this->db->From("users");
    	$this->db->where(array("id"=>$_REQUEST['user'],"role_id"=>$_REQUEST['role']));
    	$this->db->Select ();
    	$result1 = $this->db->resultArray ();
    	/* echo $this->db->lastQuery();
    	print_r($result1);die; */
    	...
    	for($i=0;$i< count($result);$i++)
    	{
    		...
    		{
    			if($result[$i]['screen_id']==$result1[$j]['screen_id'])
    			{
    				$result[$i]['permit']=$result1[$j]['permit'];
    			}
    		}
    	...
    	$_SESSION['count'] = count($result1);
    	return array("permission"=>$result,"screens"=>$result2); 
